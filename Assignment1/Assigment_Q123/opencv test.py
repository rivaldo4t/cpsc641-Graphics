...
for i in range(9):
    ...
    # m_filter = gen_box_cir(m_filter_size)
    # m_filter = gen_tri_sq(m_filter_size)
    # m_filter = gen_tri_cir(m_filter_size)
...
cv2.imwrite(file_name + '_levels' + ext, img2)
...
'''
f = 4
m_filter = gen_box_sq(f)
h = img.shape[0]
w = img.shape[1]
c = img.shape[2]
temp_img = np.ones(img.shape, np.uint8)
for y in range(0, h):
    for x in range(0, w):
        weighted_sum = 0
        f_sum = 0
        for fy in range(f):
            for fx in range(f):
                weighted_sum = weighted_sum + m_filter[fy, fx].astype(np.uint64) * img[y, x].astype(np.uint64)
                f_sum = f_sum + m_filter[fy, fx].astype(np.uint64)
                temp_img[y, x] = weighted_sum / f_sum

cv2.imshow('Mils1', img)
cv2.imshow('Mils2', temp_img)
cv2.waitKey(0)
cv2.destroyAllWindows()
'''