...
pts1 = np.float32([[0,0],[0,512],[512,0],[512,512]])
pts2 = np.float32([[85,170],[85,340],[425,85],[425,425]])
pts2 = np.float32([[170,85],[340,85],[85,425],[425,425]])
M = cv2.getPerspectiveTransform(pts1,pts2)
dst = cv2.warpPerspective(img,M,(cols,rows))
cv2.imshow('MipMap Levels', dst)
'''
...
imgx = cv2.imread('photo_512x512.jpeg')
while True:
    cv2.imshow('lalascasa', imgx)

    k = cv2.waitKey()
    print k

    if k == ord('w'):
        imgx = cv2.imread('photo_512x512_2.jpeg')
    elif k == ord('s'):
        imgx = cv2.imread('photo_512x512_4.jpeg')
    elif k == ord('q'):
        break
